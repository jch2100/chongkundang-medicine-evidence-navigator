...
작성일: 2026-09-22 · 갱신: 2026-09-23
대상: `chongkundang-medicine-evidence-navigator`
...
| 단계 | 내용 | 상태 |
|---|---|---|
| S0 | 스키마·코드체계 문서화 | **완료** — `docs/DATA_DICTIONARY.md`, `docs/KCD_MAPPING.md` |
| S1 | 코드 백본 (브랜드→품목→청구단위) | **완료** — 품목 99건·포장 311건·급여 등재 71건 |
| S2 | 상병코드 매핑 + 급여기준 | **부분 완료** — 19개 브랜드·완전코드 889개 생성, 전체 검토 전. 급여기준은 2건만 확인 |
| S3 | 처방 실무 (DUR·마약류·용량조절) | **미착수** — 인증키 확보 완료, 수집 스크립트 필요 |
| S4 | 연구 레이어 재구축 | **완료** — 성분 20종 검색식, 문헌 69건(AI 검증 62) |
| S5 | 화면·검증·문서 정합 | **완료** — 함량별 표·코드 복사·역검색·상병코드·급여기준·검색경로, 검증기 v2, 문서 정리 |

이 문서는 계획이고, 현재 구현 상태는 [README.md](README.md)가 단일 출처다.

### 계획 수립 당시 예상과 달랐던 것

- **보험코드 선행 0 탈락** — 약가마스터는 8자리, 급여목록표는 9자리로 저장한다. 패딩 없이 조인하면 6품목이 조용히 미등재로 떨어진다.
- **상병코드의 완전/불완전 구분** — `I10`·`M81`은 단독 청구가 불가능하다. 계획서에는 이 구분이 없었고, 모르고 진행했으면 틀린 코드를 현장에 내보냈을 것이다.
- **공동판매 품목의 키 구조** — 품목기준코드는 공유하고 표준코드만 업체별로 다르다. 품목기준코드 단독을 키로 쓰면 두 회사 품목이 한 덩어리로 합쳐진다.
- **검토 게이트의 현실** — 임상 전문가 검토자가 없는 상태에서 "사람 검토 후 공개" 규칙은 지킬 수 없다. 검토 주체를 라벨로 밝히는 방식(`reviewType`)으로 대체했다.
