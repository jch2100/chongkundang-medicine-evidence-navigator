...
if (!Array.isArray(products.products) || products.products.length !== 20) errors.push('products.json: ETC MVP product count must be exactly 20');
const ids = new Set();
...
for (const product of products.products || []) {
  for (const field of ['id', 'name', 'dartRank', 'dartCategory', 'status', 'sources']) {
  ...
  if (!core.items?.[product.id]) errors.push(`product:${product.id} missing official-core record`);
}
...
if (run.productsTotal !== products.products.length) errors.push('latest run product count mismatch');
...
console.log(`VALIDATION PASSED · products=${products.products.length} · changes=${changes.changes.length} · literature=${literature.items.length} · reviewQueue=${run.failures.length}`);
